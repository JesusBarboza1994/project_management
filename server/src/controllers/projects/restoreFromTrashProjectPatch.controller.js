import { restoreFromTrashProject } from "../../services/project/restoreFromTrashProject.service.js";

export default async function restoreFromTrashProjectPatchController(req, res) {
  try {
    const id = req.params.id;
    await restoreFromTrashProject({ id })
    res
      .status(200)
      .json({ message: "Proyecto enviado/regresado a la papelera" });
  } catch (error) {
    console.log(error);
    if(error.status >= 400 && error.status < 500) return res.status(400).send({ success: false, errors: error.message, code: error.code })
    return res.status(500).send({ success: false, errors: error.message })  
  }
}